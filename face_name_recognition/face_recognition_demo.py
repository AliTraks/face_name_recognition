...
class RealtimePipeline:
    """Threaded pipeline for real-time webcam processing"""
    ...
    def __init__(self, config: Config, recognizer: FaceRecognizerEngine):
        ...
        self.skip_frames = config.get('realtime', 'skip_frames')
    ...
    def _process_loop(self):
        ...
        while self.running:
            ...
            if self.frame_counter % (self.skip_frames + 1) == 0:
                face_locations = self.recognizer.detect_faces(rgb_frame)
                results = []
                ...
                try:
                    self.result_queue.put_nowait((frame, results))
                except:
                    pass
        ...
        if self.pipeline and self.pipeline.running:
            ...
            if result is not None:
                ...
                for res in results:
                    ...
                    cv2.rectangle(rgb_frame, (left, bottom), (right, bottom + 25), color, -1)
                    cv2.putText(rgb_frame, label, (left + 6, bottom + 18), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
